function [J_direction] = derive_farfield(p,h,Var,M,Z)
...
A = Var.A;
num_x = length(p);
[p_in_between,w,Polp_1,Polp_2,ts,der_p] = SetupFarField(p,M,num_x,mu_rel,eps_rel);
[h_in_between,~,~,~,~,der_h] = SetupFarField(h,M,num_x,mu_rel,eps_rel,ts);
const_mu = 2/(mu_rel+1);
...
for j=1:size(Z,2)
    ...
    %% The first integral
    % the integrand
    Help1 = roh^2*pi*(-KA^2)*(mu_rel-1)*(integrand1 + integrand2 + integrand3+ integrand4);
    ...
    %% The second integral
    Help2= -roh^2*pi*KA^2*(1-eps_rel)*(integrand5 + integrand6 + integrand7 + integrand8);
