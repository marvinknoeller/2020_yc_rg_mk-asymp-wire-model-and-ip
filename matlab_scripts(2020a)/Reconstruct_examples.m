% This script sets up the whole algorithm. We chose the example we want to
...
if Example==3
    ...
    z0=h*t;
    points=[x0;y0;z0];
    load('farFieldDhelix_new003.mat');
    FF=FarField.';
end
...
A=[A,X];
arrow(1)=1;
[XX,errN,errD,errU,errG,arrnum]=inv_test_maxwell_spline(Var,X,FF);
A=[A,XX{2:end}];
if save_results==1
...
if movie==1
    ...
    mov = VideoWriter(name,'Motion JPEG AVI');
    mov.Quality=100;
    mov.FrameRate = 10;
    open(mov);
    for k=1:length(A)
