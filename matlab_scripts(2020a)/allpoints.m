...
if trapez == 1
    ...
    val = zeros(3,(n-1)*(2*M-1) - (n-2));
    derval = zeros(3,(n-1)*(2*M-1) - (n-2));
    derderval =  zeros(3,(n-1)*(2*M-1) - (n-2));
    t = zeros(1,(n-1)*(2*M-1) - (n-2));
    for ite = 1:n-1 %loop over all segments
        xx = linspace(ts(ite),ts(ite+1),(2*M-1));
        t((2*M-1)*(ite-1)+1-(ite-1):(2*M-1)*ite-(ite)) = xx(1:end-1);
        
        [v1,dv1,ddv1] = splineeval(c1(ite,:),ts(ite),xx);
        [v2,dv2,ddv2] = splineeval(c2(ite,:),ts(ite),xx);
        [v3,dv3,ddv3] = splineeval(c3(ite,:),ts(ite),xx);
        
        val(1,(2*M-1)*(ite-1)+1-(ite-1):(2*M-1)*ite-(ite)) = v1(1:end-1);
        val(2,(2*M-1)*(ite-1)+1-(ite-1):(2*M-1)*ite-(ite)) = v2(1:end-1);
        val(3,(2*M-1)*(ite-1)+1-(ite-1):(2*M-1)*ite-(ite)) = v3(1:end-1);
        
        derval(1,(2*M-1)*(ite-1)+1-(ite-1):(2*M-1)*ite-(ite)) = dv1(1:end-1);
        derval(2,(2*M-1)*(ite-1)+1-(ite-1):(2*M-1)*ite-(ite)) = dv2(1:end-1);
        derval(3,(2*M-1)*(ite-1)+1-(ite-1):(2*M-1)*ite-(ite)) = dv3(1:end-1);
        
        derderval(1,(2*M-1)*(ite-1)+1-(ite-1):(2*M-1)*ite-(ite)) = ddv1(1:end-1);
        derderval(2,(2*M-1)*(ite-1)+1-(ite-1):(2*M-1)*ite-(ite)) = ddv2(1:end-1);
        derderval(3,(2*M-1)*(ite-1)+1-(ite-1):(2*M-1)*ite-(ite)) = ddv3(1:end-1);
    end
    t(end) = xx(end);
